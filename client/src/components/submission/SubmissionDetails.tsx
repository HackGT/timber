...
const SubmissionDetails: React.FC = props => {
  const { submissionId } = useParams<any>();
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", width: "100%" }}>
      ...
      <Title level={2} style={{ margin: "30px 0" }}>
        {data.submission.hackathon} Submission Details
      </Title>
      <Descriptions layout="vertical">
        ...
        <Descriptions.Item label={<Label name="Emails" />}>
          {data.submission.members.map((item: any) => item.email).join(", ")}
        </Descriptions.Item>
        <Descriptions.Item label={<Label name="Devpost" />}>
          <a href={data.submission.devpost}>{data.submission.devpost}</a>
        </Descriptions.Item>
        <Descriptions.Item label={<Label name="Selected Prizes" />}>
          {data.submission.prizes.join(", ")}
        </Descriptions.Item>
      </Descriptions>
